...
func (s *stream) HandleVideoCreated(ctx context.Context, req *pb.HandleVideoCreatedRequest) (*emptypb.Empty, error) {
	...
	if req.GetScale() != 0 {
		...
		err = s.handleVideoWithVariant(ctx, id, variant, req.GetUrl())
		if err != nil {
	...
	variants := []int{360, 480, 720, 1080}
	for _, scale := range variants {
		err := s.produceVideoCreatedWithScaleEvent(
			&pb.HandleVideoCreatedRequest{
				Id:    req.GetId(),
				Url:   req.GetUrl(),
				Scale: int32(scale),
			},
...
func (s *stream) produceVideoCreatedWithScaleEvent(req *pb.HandleVideoCreatedRequest) error {
	...
	err = s.producer.SendMessages(msgs)
	if err != nil {
		return err
	}
